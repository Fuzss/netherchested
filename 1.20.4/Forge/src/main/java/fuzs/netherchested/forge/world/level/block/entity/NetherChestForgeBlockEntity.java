package fuzs.netherchested.forge.world.level.block.entity;

import fuzs.limitlesscontainers.forge.api.limitlesscontainers.v1.LimitlessInvWrapper;
...
public class NetherChestForgeBlockEntity extends NetherChestBlockEntity {
    ...
    public <T> LazyOptional<T> getCapability(Capability<T> cap, Direction side) {
        if (!this.remove && cap == ForgeCapabilities.ITEM_HANDLER) {
            if (this.chestHandler == null)
                this.chestHandler = LazyOptional.of(() -> new LimitlessInvWrapper(this.getContainer()));
