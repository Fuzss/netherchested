            ...
            public void handle(ServerboundContainerClickMessage message, Player player, Object instance) {
                ...
                if (player.containerMenu.containerId == message.getContainerId()) {
                    if (player.isSpectator()) {
                        ...
                        NetherChest.LOGGER.debug("Player {} interacted with invalid menu {}", player, player.containerMenu);
                    } else {
                        int i = message.getSlotNum();
                        if (!player.containerMenu.isValidSlotIndex(i)) {
                            NetherChest.LOGGER.debug("Player {} clicked invalid slot index: {}, available slots: {}", player.getName(), i, player.containerMenu.slots.size());
                        } else {
